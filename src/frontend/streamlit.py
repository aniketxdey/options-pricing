...
# BACKTESTING RESULTS PAGE
    ...
    # Try to load existing results if not already loaded
    if not hasattr(st.session_state, 'backtest_completed') or not st.session_state.backtest_completed:
        try:
            import os
            ...
            pass  # Silently fail if no existing results
    ...
    with col2:
        ...
        # Display results if available
            ...
            # Success message - check if this is from a fresh run or loaded results
            if hasattr(st.session_state, 'backtesting_running') and not st.session_state.backtesting_running:
                st.success("Backtesting completed successfully!")
            else:
                st.info("📊 Showing results from previous backtesting run")
